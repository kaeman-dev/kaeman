...
export const simulateVg = async (ctx: Context, config: Config): Promise<SimResult> => {
  ...
  const drops: Item[] = [];
  const rolls = randInt(vgLoot.rolls) + (perk ? vgLoot.perk.bonusRolls : 0);
...
&a&l  REWARDS
${lines}    ${vgLoot.powder.name} &8x${Math.floor(randInt(vgLoot.powder.quantity) * vgLoot.powder.multiplier)}
