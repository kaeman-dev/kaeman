...
const parseLine = (line: string): Segment[] => {
  ...
  for (let i = 0; i < line.length; i++) {
    ...
    if ((line[i] === "&" || line[i] === "§") && code && (colors[code] || "lmnor".includes(code))) {
      flush();
      const nextColor = colors[code];
      if (nextColor) {
        color = nextColor;
        bold = false;
      } else if (code === "l") bold = true;
      else if (code === "r") {
        color = minecraftColors.f;
        bold = false;
      }
      i++;
    } else buffer += line[i];
...
export const renderMinecraft = async (
  result: Pick<SimResult, "type" | "text">,
): Promise<Buffer> => {
  const lines = result.text.split("\n").map((line) => line.split("\t").map(parseLine));
  const width = 1920;
  const padding = 16;
  ...
  for (const candidate of renderSettings.fontSizes) {
    ...
    const tableWidth =
      columns.reduce((sum, width) => sum + width, 0) + Math.max(0, columns.length - 1) * candidate;
    const maxWidth = Math.max(tableWidth, ...widths.filter((row) => row.length === 1).flat());
    if (maxWidth + padding * 2 > width && candidate > minSize) break;
    size = candidate;
  ...
  const bg = await loadImage(join(assets, result.type));
