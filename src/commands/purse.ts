...
import type { FF1Codec } from "../utils/ff1";
  ...
  command
    ...
    .action(async ({ session }, page = 1) => {
      ...
      const text = [
        ...
        `&7User ID: ${userIds.encrypt(session.user.id)} | UTC+8`,
        "",
        ...result.records.map(({ id, deltaCents, source, createdAt }) => {
          ...
          return `&7${time}\t&b| ${source}\t&8| #${historyIds.encrypt(id)}\t&7| ${sign} ${compact.format(Math.abs(deltaCents) / 100)}`;
        }),
        "",
